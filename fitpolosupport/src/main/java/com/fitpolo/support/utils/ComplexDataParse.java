...
public class ComplexDataParse {
    ...
    public static void parseStep(byte[] value, ArrayList<Step> steps) {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm");
        Calendar calendar = Calendar.getInstance();
        for (int i = 0; i < 2; i++) {
            byte year = value[i * 7 + 4];
            byte month = value[i * 7 + 5];
            byte day = value[i * 7 + 6];
            byte hour = value[i * 7 + 7];
            byte min = value[i * 7 + 8];
            int stepValue = DigitalConver.byte2Int(value[i * 7 + 10])+256*DigitalConver.byte2Int(value[i * 7 + 9]) ;
            if (DigitalConver.byte2Int(year) == 0) {
                continue;
            }
            calendar.set(2000 + DigitalConver.byte2Int(year),
                    ...
                    DigitalConver.byte2Int(min));
            Date time = calendar.getTime();
            String stepTime = sdf.format(time);
            String stepStr = stepValue + "";
            Step step = new Step();
            step.time = stepTime;
            step.value = stepStr;
